...
#   0 3 * * * /opt/outbreak/scripts/backup-postgres.sh
#
# Restore with: pg_restore --clean --if-exists -d "$DATABASE_URL" <file>
set -euo pipefail

: "${DATABASE_URL:?DATABASE_URL must be set}"
BACKUP_DIR="${BACKUP_DIR:-/var/backups/outbreak}"
RETENTION_DAYS="${RETENTION_DAYS:-14}"

mkdir -p "$BACKUP_DIR"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
out="$BACKUP_DIR/outbreak-$stamp.dump"

echo "Writing $out"
pg_dump --format=custom --no-owner --no-acl --dbname="$DATABASE_URL" > "$out"

echo "Pruning backups older than $RETENTION_DAYS days"
find "$BACKUP_DIR" -maxdepth 1 -type f -name 'outbreak-*.dump' -mtime +"$RETENTION_DAYS" -delete
